"""
Skill 加载器

从目录加载 Skill 定义（SKILL.md 文件）。

Skill 目录结构：
    skills/
    ├── pdf/
    │   └── SKILL.md
    ├── xlsx/
    │   └── SKILL.md
    └── docx/
        └── SKILL.md

SKILL.md 格式（YAML Front Matter）：
    ---
    name: pdf
    description: PDF 文件处理
    type: task
    version: 1.0.0
    ---
    
    # PDF Skill
    
    详细说明...

使用方式：
    loader = SkillLoader(Path("./skills"))
    skills = loader.list_skills()
    desc = loader.get_all_descriptions()
"""

from __future__ import annotations

import logging
from dataclasses import dataclass
from pathlib import Path

import yaml

logger = logging.getLogger(__name__)


@dataclass
class SkillMeta:
    """
    Skill 元数据
    
    从 SKILL.md 的 YAML Front Matter 解析。
    
    Attributes:
        name: Skill 名称
        description: 简短描述
        type: 类型（task, chat 等）
        version: 版本号
        path: Skill 目录路径
    """
    name: str
    description: str
    type: str
    version: str
    path: Path

    @classmethod
    def from_md(cls, path: Path) -> "SkillMeta":
        """
        从 SKILL.md 解析元数据
        
        Args:
            path: SKILL.md 文件路径
            
        Returns:
            SkillMeta 对象
            
        Raises:
            ValueError: 文件格式无效时抛出
        """
        content = path.read_text(encoding="utf-8")

        # 检查 YAML Front Matter
        if not content.startswith("---"):
            raise ValueError(f"Invalid SKILL.md (missing front matter): {path}")

        parts = content.split("---", 2)
        if len(parts) < 3:
            raise ValueError(f"Invalid SKILL.md (malformed front matter): {path}")

        frontmatter = yaml.safe_load(parts[1])

        return cls(
            name=frontmatter.get("name", path.parent.name),
            description=frontmatter.get("description", ""),
            type=frontmatter.get("type", "task"),
            version=frontmatter.get("version", "0.1.0"),
            path=path.parent,
        )


class SkillLoader:
    """
    Skill 加载器
    
    扫描目录并加载所有 Skill 定义。
    
    使用方式：
        loader = SkillLoader(Path("./skills"))
        for skill in loader.list_skills():
            print(f"- {skill.name}: {skill.description}")
    """

    def __init__(self, skills_dir: Path):
        """
        初始化加载器
        
        Args:
            skills_dir: Skills 目录路径
        """
        self._skills_dir = skills_dir
        self._skills: dict[str, SkillMeta] = {}
        self._load_skills()

    def _load_skills(self) -> None:
        """加载所有 Skill"""
        if not self._skills_dir.exists():
            logger.warning("Skills dir not found: %s", self._skills_dir)
            return

        for skill_dir in self._skills_dir.iterdir():
            if not skill_dir.is_dir():
                continue

            skill_md = skill_dir / "SKILL.md"
            if not skill_md.exists():
                continue

            try:
                meta = SkillMeta.from_md(skill_md)
                self._skills[meta.name] = meta
                logger.info("Loaded skill: %s", meta.name)
            except Exception:
                logger.warning("Failed to load skill: %s", skill_dir)

    def list_skills(self) -> list[SkillMeta]:
        """
        列出所有 Skill
        
        Returns:
            SkillMeta 列表
        """
        return list(self._skills.values())

    def get_skill(self, name: str) -> SkillMeta | None:
        """
        获取指定 Skill
        
        Args:
            name: Skill 名称
            
        Returns:
            SkillMeta 对象，不存在则返回 None
        """
        return self._skills.get(name)

    def get_all_descriptions(self) -> str:
        """
        获取所有 Skill 的描述文本
        
        用于构建系统提示。
        
        Returns:
            格式化的描述文本
        """
        if not self._skills:
            return "暂无可用 Skill"

        lines = ["可用 Skill：\n"]
        for skill in self._skills.values():
            lines.append(f"- **{skill.name}**: {skill.description}")

        return "\n".join(lines)
