...
提供统一的 LLM 模型配置管理，支持多 Provider。
主要功能：
- 多 Provider 支持（aliyun, openrouter 等）
- 模型白名单限制
- 角色模型映射
- 自动加载 .env 文件
"""
    ...
    def _get_default_config(self) -> dict:
        ...
        return {
            "default": {
                "provider": "aliyun",
                "text_model": "qwen-max",
                "image_model": "qvq-plus",
                "summary_model": "qwen3-turbo",
                "temperature": 0.7,
                "retry_count": 2,
                "timeout": 600,
            },
    ...
    def validate_model(self, model: str) -> None:
        ...
        if not self.is_model_allowed(model):
            ...
            raise ValueError(
                ...
                f"请在 jackclaw/llm/llm_config.yaml 中添加此模型，"
