"""Main Crew — jackclaw 主协调 Crew（with Memory）
...
from jackclaw.memory.bootstrap import build_bootstrap_prompt
from jackclaw.memory.context_mgmt import (
    append_session_raw,
    load_session_ctx,
    maybe_compress,
    prune_tool_results,
...
from jackclaw.memory.indexer import async_index_turn
from jackclaw.models import SenderProtocol
from jackclaw.runner import AgentFn
from jackclaw.session.models import MessageEntry
from jackclaw.tools.intermediate_tool import IntermediateTool
from jackclaw.llm import LLMFactory, llm_config
logger = logging.getLogger(__name__)

# SkillLoaderTool 在模块顶层导入，方便测试 mock
try:
    from jackclaw.tools.skill_loader_tool import SkillLoaderTool  # noqa: E402
except ImportError:
...
def _load_available_skills(skills_config_path: Path | None = None) -> str:
    ...
    Args:
        skills_config_path: 技能配置文件路径，默认使用项目默认路径

    Returns:
        格式化的技能列表字符串
    """
    if skills_config_path is None:
        # 默认路径：jackclaw/skills/load_skills.yaml
        repo_root = Path(__file__).resolve().parents[2]
        skills_config_path = repo_root / "jackclaw" / "skills" / "load_skills.yaml"

    if not skills_config_path.exists():
        return "当前无可用技能（配置文件不存在）"

    try:
        config = yaml.safe_load(skills_config_path.read_text(encoding="utf-8"))
        ...
        for skill in enabled_skills:
            ...
            skill_info = f"- **{name}**: {desc} [{skill_type}]"
            if skill_type == "reference":
                ref_skills.append(skill_info)
            else:
                task_skills.append(skill_info)

        # 构建格式化输出
        lines = []

        if ref_skills:
            lines.append("**Reference 类型（返回操作规范，供参考）：**")
            lines.extend(ref_skills)
            lines.append("")

        if task_skills:
            lines.append("**Task 类型（在沙盒中执行，返回结果）：**")
            lines.extend(task_skills)
            lines.append("")

        # 添加使用提示
        lines.append("**使用提示：**")
        lines.append("- 调用 reference 类型：`SkillLoaderTool(skill_name=\"名称\")`")
        lines.append("- 调用 task 类型：`SkillLoaderTool(skill_name=\"名称\", task_context=\"任务描述\")`")

        return "\n".join(lines)

    except Exception as e:
        logger.warning("Failed to load skills list: %s", e)
        return f"加载技能列表失败：{e}"
...
class MemoryAwareCrew:
    ...
    def orchestrator(self) -> Agent:
        ...
        # 💡【技能列表注入】将可用技能列表注入到 backstory，让 Agent 知道有哪些技能可用
        ...
        if SkillLoaderTool is not None:
            ...
            logger.info("✓ SkillLoaderTool loaded (config: %s)", skills_config_path)

            # 输出可用的技能列表
            skills = skill_loader._load_skills_config()
            task_count = sum(1 for s in skills.values() if s.get("type") == "task")
            ref_count = sum(1 for s in skills.values() if s.get("type") == "reference")
            logger.info("  └─ 可用技能: %d 个 task 类型, %d 个 reference 类型", task_count, ref_count)
        else:
            logger.warning("✗ SkillLoaderTool not available, skills will not work")

        # 添加中间结果保存工具
        tools.append(IntermediateTool())
        logger.info("✓ IntermediateTool loaded (保存中间结果)")

        # 🔍 调试日志：输出传递给 Agent 的工具列表
        logger.info("=" * 60)
        logger.info("🔧 传递给 Agent 的 Tools 列表:")
        for i, tool in enumerate(tools):
            tool_name = getattr(tool, 'name', 'unknown')
            tool_desc = getattr(tool, 'description', '')[:100]  # 只显示前100字符
            logger.info("  [%d] %s", i + 1, tool_name)
            logger.info("      描述: %s...", tool_desc)
        logger.info("=" * 60)

        # 💡【第07课·max_iter】设置最大迭代次数，防止无限循环
        cfg["max_iter"] = cfg.get("max_iter", 3)  # 默认最多3次迭代
