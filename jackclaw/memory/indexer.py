...
def _get_llm_client():
    ...
    if _llm_client is None:
        _llm_client = LLMFactory.create_for_role("assistant")
    return _llm_client
...
def extract_summary_and_tags(
    ...
    raw = client.call(messages=messages).strip()
...
def _index_single_turn(
    session_id:      str,      # 会话ID，用于标识特定的对话会话
    routing_key:     str,      # 路由键，可能用于消息路由或分类
    user_message:    str,      # 用户发送的消息内容
    assistant_reply: str,      # 助手的回复内容
    turn_ts:         int,      # 对话轮次的时间戳
    db_dsn:          str,      # 数据库连接字符串(Data Source Name)
) -> None:
