...
def chunk_by_tokens(
    ...
    # 初始化存储所有分块的列表、当前分块和当前分块的token计数
...
def _summarize_chunk(messages: list[dict]) -> str:
    ...
    try:
        ...
        from jackclaw.llm import LLMFactory        
        summary_llm = LLMFactory.create_for_role("assistant")
        history = "\n".join(
