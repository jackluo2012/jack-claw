...
from jackclaw_team.agents.skill_crew import build_skill_crew
...
_SKILLS_DIR = Path(__file__).parents[2] / "jackclaw_team" / "skills"
...
class SkillLoaderTool(BaseTool):
    ...
    # 💡 v29 课并发修复：skills_dir / sandbox_mount 改为**实例级**，
    # 不再依赖模块级 `_SKILLS_DIR` 全局变量（4 role 并发时会 race）。
    _skills_dir: Path = PrivateAttr(default=None)  # type: ignore[assignment]
    _sandbox_mount: str = PrivateAttr(default="")
    ...
    def __init__(
        ...
        # 路径：显式传入的参数优先，否则回落到模块默认
        self._skills_dir = skills_dir if skills_dir is not None else _SKILLS_DIR
        self._sandbox_mount = sandbox_mount if sandbox_mount is not None else _SANDBOX_SKILLS_MOUNT
    ...
    def _build_description(self) -> None:
        ...
        _skills_root = self._skills_dir.resolve()
        ...
        for skill_conf in skills_conf:
            ...
            skill_path = (self._skills_dir / name).resolve()
    ...
    def _get_skill_instructions(self, skill_name: str) -> str:
        ...
        # 沙盒路径常量
        _skill_base = f"{self._sandbox_mount}/{skill_name}"
        ...
        sandbox_directive = (
            ...
            f"【目录初始化】第一步必须先确保工作目录存在，否则后续文件操作会报错：\n"
            f"  sandbox_execute_bash cmd=\"mkdir -p {_session_dir}/uploads {_session_dir}/outputs {_session_dir}/tmp\"\n"
